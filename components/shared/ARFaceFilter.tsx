...
const ARFaceFilter = forwardRef<ARFaceFilterHandle, ARFaceFilterProps>(
  (
    ...
    const startAR = useCallback(async () => {
      ...
      try {
        ...
        if (video) {
          ...
          bgMaterialRef.current.uniforms.tDiffuse.value =
        ...
        renderer.setAnimationLoop(() => {
          ...
          if (
            ...
            bgMaterialRef.current.uniforms.uVideoScale.value.set(
              scaleX,
              scaleY,
            );
            bgMaterialRef.current.uniforms.uVideoOffset.value.set(
              offsetX,
              offsetY,
            );
            ...
            const projectAnchor = (index: number) => {
              ...
              const vec = new THREERef.current.Vector3().setFromMatrixPosition(
    ...
    const applyFaceMesh = useCallback(
      async (textureUrl: string): Promise<void> => {
        const THREE = THREERef.current;
    ...
    useEffect(() => {
      ...
      const syncFilters = async () => {
        ...
        if (activeFaceMesh && currentFaceMeshId !== activeFaceMesh.id)
          needsLoading = true;
        ...
        activeFilters.forEach((filter) => {
          if (filter.type === "facemesh" || filter.type === "distortion")
            return;

          if (!meshesRef.current.has(filter.id)) {
            const createFn =
              filter.type === "model" ? createModelMesh : createOverlayMesh;
        ...
        const activeDistortion = activeFilters.find(
          (f) => f.type === "distortion",
        );
        if (activeDistortion && activeDistortion.distortionConfig) {
          ...
          if (bgMaterialRef.current)
            bgMaterialRef.current.uniforms.uRadii.value.fill(0);
          const video = containerRef.current?.querySelector("video");
