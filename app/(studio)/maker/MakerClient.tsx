...
const OUTLINE_GRADIENTS: GradientPreset[] = [
  ...
  },
  {
    ...
    css: 'linear-gradient(135deg, #39ff14, #00e5ff)',
  },
  {
    id: 'fire',
    label: 'Fire',
    colors: ['#FF6B4A', '#dc2626'],
    css: 'linear-gradient(135deg, #FF6B4A, #dc2626)',
  },
  {
    id: 'gold',
    label: 'Gold',
...
export default function MakerPage() {
  ...
  const updateSelectedText = useCallback(
    (patch: Partial<CanvasElement>) => {
      ...
      setElements((prev) =>
        prev.map((el) => (el.id === selectedId && el.type === 'text' ? { ...el, ...patch } : el)),
      )
    },
  ...
  useEffect(() => {
    ...
    generateOutline(processedUrl, outlineWidth, shadowBlur, outlineColor, outlineGradient)
      ...
      .catch((err) => {
        ...
        if (isMounted) {
          setError('Failed to generate sticker outline')
          setIsProcessingCanvas(false)
        }
      })
    return () => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      const cleanMsg = msg.includes('{')
        ? msg.split('{')[0].trim() || 'Background removal failed.'
        : msg.length > 120
          ? msg.slice(0, 120) + '...'
          : msg
      setError(cleanMsg)
  ...
  const handleCanvasExport = async (dataUrl: string) => {
    ...
    if (action === 'download') {
      ...
      try {
        ...
        toast(
          `Exported for ${result.format.label} (${sizeStr}) ${statusMsg}`,
          result.withinLimits ? 'success' : 'info',
        )
      } catch (err) {
  ...
  const duplicateElement = (id: string) => {
    ...
    setElements((prev) => [
      ...prev,
      { ...el, id: `${el.type}-${Date.now()}`, x: el.x + 20, y: el.y + 20 },
    ])
  ...
  const handleApplyFilter = async (filterId: string) => {
    ...
    try {
      if (filterId === 'none') {
        ...
        const cleanUrl = await generateOutline(
          processedUrl,
          outlineWidth,
          shadowBlur,
          outlineColor,
        ...
        setElements((prev) =>
          prev.map((e) => (e.type === 'main-sticker' ? { ...e, src: cleanUrl } : e)),
        )
      } else {
        const filteredUrl = await applyFilterToImage(processedUrl, filterId)
        setElements((prev) =>
          prev.map((e) => (e.type === 'main-sticker' ? { ...e, src: filteredUrl } : e)),
        )
      }
    } catch (err) {
  ...
  const handleApplyFrame = async (frameId: string) => {
    ...
    try {
      if (frameId === 'none') {
        ...
        const framedUrl = await applyFrameToImage(
          processedUrl,
          frameId,
          outlineColor,
          frameBorderWidth,
        ...
        setElements((prev) =>
          prev.map((e) => (e.type === 'main-sticker' ? { ...e, src: framedUrl } : e)),
        )
      }
    } catch (err) {
  ...
  const handleSmartCrop = async () => {
    ...
    try {
      ...
      setElements((prev) =>
        prev.map((e) => (e.type === 'main-sticker' ? { ...e, src: result.dataUrl } : e)),
      )
  ...
  return (
    <div className="flex-1 flex overflow-hidden relative">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-2 py-1.5 bg-[var(--input-bg)] border border-(--overlay-border) rounded-lg text-xs text-(--text-secondary) focus:border-[#FF6B4A]/30 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    return (
                      <div key={el.id}>
                        ...
                        <div
                          draggable
                          onDragStart={() => {
                            dragRef.current = { dragIdx: realIdx, overIdx: realIdx }
                          }}
                          onDragOver={(e) => {
                            e.preventDefault()
                        ...
                        >
                          ...
                          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="w-5 h-5 rounded flex items-center justify-center hover:bg-(--card-bg-hover) cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="w-5 h-5 rounded flex items-center justify-center text-red-400 hover:bg-red-400/10 cursor-pointer"
                            >
            ...
            <div className="space-y-4">
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-semibold uppercase transition-all cursor-pointer ${
                        exportFormat === fmt
                          ...
                          : 'bg-[var(--input-bg)] text-(--text-muted) hover:bg-(--card-bg-hover) hover:text-(--text-secondary)'
                      }`}
                    >
                      {fmt}
                    </button>
                ...
                <PanelSection title="Quality">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="flex-1"
                      style={{
                        background: `linear-gradient(to right, #FF6B4A ${exportQuality}%, rgba(255,255,255,0.06) ${exportQuality}%)`,
                      }}
                    />
              ...
              <PanelSection title="Canvas Info">
                <div className="space-y-1.5 text-xs text-(--text-muted)">
                  ...
                  <div className="flex justify-between">
                    <span>Elements</span>
                    <span className="text-(--text-secondary)">{elements.length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Zoom</span>
          ...
          <div className="absolute bottom-6 right-6 z-50 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 bg-[var(--panel-bg)] p-2 rounded-2xl shadow-lg border border-[var(--overlay-border)]">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-xl bg-[var(--card-bg)] hover:bg-[var(--card-bg-hover)] text-[var(--text-secondary)] transition-all cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-2 bg-[var(--panel-bg)] p-2 rounded-2xl shadow-xl border border-[var(--overlay-border)]">
              ...
              <div className="text-[10px] font-bold text-center text-[var(--text-muted)] w-8">
                {zoom}%
              </div>
              <button
              ...
              <PanelSection title="Outline Color">
                  ...
                  {OUTLINE_PRESETS.map((p) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setOutlineGradient(null)
                      }}
                      className={`w-full aspect-square rounded-lg border-2 transition-all cursor-pointer ${
                        outlineColor === p.color && !outlineGradient
                          ? 'border-[#FF6B4A] scale-110'
                          : 'border-(--overlay-border) hover:border-(--overlay-border-hover)'
                      }`}
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-8 h-8 rounded-lg cursor-pointer border border-(--overlay-border)"
                  />
                  <input
                    type="text"
              ...
              <PanelSection title="Outline Width">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1"
                    style={{
                      background: `linear-gradient(to right, #FF6B4A ${outlineWidth * 2}%, rgba(255,255,255,0.06) ${outlineWidth * 2}%)`,
                    }}
                  />
                  <span className="text-[11px] text-(--text-tertiary) font-mono w-8 text-right">
                    {outlineWidth}px
                  </span>
                </div>
              </PanelSection>
              ...
              <PanelSection title="Shadow Blur">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1"
                    style={{
                      background: `linear-gradient(to right, #FF6B4A ${shadowBlur * 2}%, rgba(255,255,255,0.06) ${shadowBlur * 2}%)`,
                    }}
                  />
                  <span className="text-[11px] text-(--text-tertiary) font-mono w-8 text-right">
                    {shadowBlur}px
              ...
              <PanelSection title="Quick Styles">
                <div className="grid grid-cols-2 gap-1.5">
                  <button
                    ...
                    className="py-2 rounded-lg bg-[var(--input-bg)] text-[11px] font-semibold text-(--text-secondary) hover:bg-(--card-bg-hover) transition-all cursor-pointer"
                  >
                    Classic
                  </button>
                  <button
                  ...
                  <button
                    onClick={() => {
                      setOutlineGradient(OUTLINE_GRADIENTS[0])
                      setOutlineWidth(20)
                      setShadowBlur(10)
                    }}
                    className="py-2 rounded-lg bg-[var(--input-bg)] text-[11px] font-semibold text-(--text-secondary) hover:bg-(--card-bg-hover) transition-all cursor-pointer"
                  ...
                  <button
                    onClick={() => {
                      setOutlineGradient(OUTLINE_GRADIENTS[3])
                      setOutlineWidth(18)
                      setShadowBlur(15)
                    }}
                    className="py-2 rounded-lg bg-[var(--input-bg)] text-[11px] font-semibold text-(--text-secondary) hover:bg-(--card-bg-hover) transition-all cursor-pointer"
              ...
              <PanelSection title="Add Text">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (isTextSelected) updateSelectedText({ text: e.target.value })
                  }}
                  ...
                  {FONTS.map((f) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (isTextSelected) updateSelectedText({ fontFamily: f.value })
                      }}
                    ...
                    >
                      ...
                      <span className="text-sm font-bold" style={{ fontFamily: f.value }}>
                        {f.sample}
                      </span>
                    </button>
                  ))}
                    ...
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setFontSize(v)
                          if (isTextSelected) updateSelectedText({ fontSize: v })
                        }}
                        className="flex-1"
                        style={{
                          background: `linear-gradient(to right, #FF6B4A ${((fontSize - 16) / 104) * 100}%, rgba(255,255,255,0.06) ${((fontSize - 16) / 104) * 100}%)`,
                        }}
                      />
                    ...
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (isTextSelected) updateSelectedText({ strokeWidth: v })
                        }}
                        className="flex-1"
                        style={{
                          background: `linear-gradient(to right, #FF6B4A ${textStrokeWidth * 10}%, rgba(255,255,255,0.06) ${textStrokeWidth * 10}%)`,
                        }}
                      />
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (isTextSelected) updateSelectedText({ fill: e.target.value })
                        }}
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (isTextSelected) updateSelectedText({ stroke: e.target.value })
                        }}
                        className="w-7 h-7 rounded cursor-pointer border border-(--overlay-border)"
                      />
                      <span className="text-[10px] uppercase text-(--text-muted) font-mono">
                    ...
                    >
                      <span
                        ...
                        style={{
                          fontFamily: preset.font,
                          color: preset.fill,
                          textShadow: `1px 1px 0 ${preset.stroke}`,
                        }}
                      >
                        {preset.label}
                      </span>
            ...
            <div className="space-y-3">
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded-lg text-[11px] font-semibold transition-all cursor-pointer ${
                      activeTemplateCategory === cat
                        ...
                        : 'text-(--text-muted) hover:text-(--text-secondary) hover:bg-[var(--input-bg)]'
                    }`}
                  >
              ...
              <div className="grid grid-cols-6 gap-1.5 max-h-[500px] overflow-y-auto">
                {TEMPLATE_CATEGORIES[activeTemplateCategory].items.map((src, idx) => (
                  <div
                    key={src}
                    className="relative aspect-square bg-(--card-bg) rounded-lg border border-(--overlay-border) flex items-center justify-center p-1 cursor-pointer hover:border-[#FF6B4A]/30 hover:bg-(--card-bg-hover) transition-all"
                    onClick={() => handleAddTemplate(src)}
                  >
                    <Image
                      unoptimized
                      src={src}
                      alt="Template category item"
                      fill
          ...
          {rightTab === 'frames' && (
            <div className="space-y-4">
              ...
              <PanelSection title="Shape">
                <div className="grid grid-cols-3 gap-1.5">
                  {STICKER_FRAMES.filter(
                    (f) => f.id === 'none' || f.category === activeFrameCategory,
                  ).map((frame) => (
                    <button
                      key={frame.id}
              ...
              <PanelSection title="Frame Border">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1"
                    style={{
                      background: `linear-gradient(to right, #FF6B4A ${frameBorderWidth * 5}%, rgba(255,255,255,0.06) ${frameBorderWidth * 5}%)`,
                    }}
                  />
                  <span className="text-[11px] text-(--text-tertiary) font-mono w-8 text-right">
                    {frameBorderWidth}px
            ...
            <div className="space-y-4">
                  ...
                  {EXPORT_FORMATS.map((format) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-semibold text-(--text-secondary) group-hover:text-white block">
                          {format.label}
                        </span>
              ...
              <PanelSection title="Animated GIF">
                    ...
                    <button
                      ...
                      className={`py-2 px-2 rounded-lg text-[11px] font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                        selectedAnimation === anim.id
                          ...
                          : 'bg-[var(--input-bg)] text-(--text-tertiary) border border-(--overlay-border) hover:bg-(--card-bg-hover) hover:text-(--text-secondary)'
                      }`}
                    >
                ...
                <Button
                  ...
                  disabled={elements.length === 0 || isExportingGif}
                >
                  {isExportingGif ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
              ...
              <PanelSection title="Smart Crop">
                <p className="text-[11px] text-(--text-muted) mb-2">
                  Auto-crop to the main subject with padding.
                </p>
                <Button
                  className="w-full"
                  onClick={handleSmartCrop}
                  disabled={!processedUrl || isSmartCropping}
                >
                  {isSmartCropping ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
      ...
      {file && (
        <MakerMobileSheet rightTab={rightTab} onTabChange={setRightTab} tabs={RIGHT_TABS}>
              ...
              <div className="space-y-4">
                <PanelSection title="Outline Width">
                  <input
                    ...
                    className="w-full"
                    style={{
                      background: `linear-gradient(to right, #FF6B4A ${outlineWidth * 2.5}%, rgba(255,255,255,0.06) ${outlineWidth * 2.5}%)`,
                    }}
                  />
                </PanelSection>
            ...
            {rightTab === 'text' && (
              <div className="space-y-3">
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={handleAddText} disabled={!stickerText}>
              ...
              <div className="space-y-3">
                <div className="flex gap-1 overflow-x-auto">
                  {FRAME_CATEGORIES.map((c) => (
                    <button
                      key={c.id}
                      onClick={() => setActiveFrameCategory(c.id)}
              ...
              <div className="space-y-3">
                <div className="flex gap-1 overflow-x-auto">
                  {Object.keys(TEMPLATE_CATEGORIES).map((cat) => (
                    <button
                      key={cat}
                      onClick={() => setActiveTemplateCategory(cat as TemplateCategory)}
                ...
                <div className="grid grid-cols-5 gap-1.5 max-h-[200px] overflow-y-auto">
                  {TEMPLATE_CATEGORIES[activeTemplateCategory].items.slice(0, 30).map((src, i) => (
                    <button
                      key={src}
                      onClick={() => handleAddTemplate(src)}
                      className="relative aspect-square rounded-lg bg-(--card-bg) border border-(--overlay-border) p-1 hover:bg-(--card-bg-hover) transition-all cursor-pointer"
                    >
                      <Image
                        unoptimized
                        src={src}
...
function MakerMobileSheet({ children, rightTab, onTabChange, tabs }: MakerMobileSheetProps) {
  ...
  return (
    <div
      ...
      style={{
        height: expanded ? '55vh' : '56px',
        transition: 'height 0.3s cubic-bezier(0.32,0.72,0,1)',
      }}
    >
      {/* Tab toolbar */}
      <div
        className="shrink-0 flex items-stretch overflow-x-auto cursor-grab active:cursor-grabbing touch-manipulation"
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2.5 text-[9px] font-semibold transition-all cursor-pointer min-w-[50px] ${
              rightTab === tab.key && expanded
                ? 'text-[#FF6B4A] bg-[#FF6B4A]/5'
                : 'text-(--text-muted)'
            }`}
          >
