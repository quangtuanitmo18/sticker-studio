        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-(--text-muted)">
                  Let AI find the best 3–5 seconds to turn into a GIF.
                </p>
                <Button
                  variant="secondary"
              ...
              <div className="rounded-xl bg-(--card-bg) border border-(--overlay-border) p-3.5 space-y-4">
                ...
                <Slider
                  ...
                  onChange={(val) =>
                    setTrim((prev) => ({ ...prev, start: Math.min(val, prev.end - 0.5) }))
                  }
                  unit="s"
                />
                <Slider
                  ...
                  onChange={(val) =>
                    setTrim((prev) => ({ ...prev, end: Math.max(val, prev.start + 0.5) }))
                  }
                  unit="s"
                />
              </div>
          ...
          {activeTab === 'stickers' && (
            <StickersTab
              ...
              setStickerOverlays={handleSetStickerOverlays}
            />
          )}
        </div>
      </aside>
          ...
          <div className="absolute bottom-6 right-6 z-50 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 bg-(--panel-bg) p-2 rounded-2xl shadow-lg border border-(--overlay-border)">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-xl bg-(--card-bg) hover:bg-(--card-bg-hover) text-(--text-secondary) transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
              >
            ...
            <div className="flex flex-col gap-2 bg-(--panel-bg) p-2 rounded-2xl shadow-xl border border-(--overlay-border)">
              ...
              <div className="text-[10px] font-bold text-center text-(--text-muted) w-8 tabular-nums">
                {Math.round(zoom * 100)}%
              </div>
              <button
          ...
          >
            ...
            >
              ...
              {textOverlays.map((layer) => {
                const getPreviewAnimationStyles = (
                  preset: string | undefined,
                ): React.CSSProperties => {
                  switch (preset) {
                    case 'bounce':
                      return { animation: 'text-bounce 2s infinite' }
                    case 'fade':
                      return { animation: 'text-fade 2s infinite' }
                    case 'slide':
                      return { animation: 'text-slide 3s infinite' }
                    case 'glow':
                      return { animation: 'text-glow 2s infinite' }
                    case 'shake':
                      return { animation: 'text-shake 0.5s infinite' }
                    case 'wave':
                      return { animation: 'text-wave 2s infinite' }
                    case 'zoom':
                      return { animation: 'text-zoom 3s infinite' }
                    case 'typewriter':
                      return { animation: 'text-typewriter 3s steps(20, end) infinite' }
                    default:
                      return {}
                  }
                }
                ...
                return (
                  <div
                    key={layer.id}
                    className={`absolute transform -translate-x-1/2 -translate-y-1/2 select-none pointer-events-auto p-2 rounded transition-colors ${
                      draggingTextId === layer.id
                        ? 'opacity-70 cursor-grabbing ring-2 ring-blue-500 bg-black/10'
                        : 'cursor-grab hover:ring-2 hover:ring-blue-500/50'
                    }`}
                    style={{
                      left: `${layer.x * 100}%`,
                      top: `${layer.y * 100}%`,
                    ...
                    onPointerDown={(e) => {
                      e.stopPropagation()
                      setDraggingTextId(`text-${layer.id}`)
                    }}
                  >
                    {layer.text}
                  </div>
        ...
        <div className="h-32 border-t border-(--overlay-border) bg-(--card-bg) p-4 shrink-0">
          <div className="flex items-center justify-between mb-2">
            ...
            <span ref={timeDisplayRef} className="text-xs tabular-nums text-(--text-secondary)">
              {metadata
                ? `${(currentTime / speed).toFixed(1)}s / ${(metadata.duration / speed).toFixed(1)}s`
                : '0.0s / 0.0s'}
            </span>
          </div>
