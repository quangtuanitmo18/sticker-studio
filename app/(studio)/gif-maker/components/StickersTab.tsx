        ...
        <div className="pt-4 border-t border-(--overlay-border) space-y-2">
          ...
          <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
            {stickerOverlays.map((layer) => (
              <div key={layer.id} className="p-2 bg-(--card-bg) border border-(--overlay-border) rounded-xl flex items-center justify-between gap-2">
                <div className="w-8 h-8 rounded-lg bg-(--background) shrink-0 flex items-center justify-center p-1 border border-(--overlay-border)">
                ...
                <button
                  onClick={() => setStickerOverlays(prev => prev.filter(s => s.id !== layer.id))}
